...
class DeviceView extends View {
	...
	InitializeSideTools() { //overrides
		...
		if ("mac address" in this.link && this.link["mac address"].v.length > 0) {
	...
	InitializeInterfaces() {
		...
		const ModeToggle = event=> {
			if (this.switchInfo.success) {
				...
				case "Speed":
					this.switchMode = 0;
					list.forEach(o=> {
						o.iconElement.style.background = "linear-gradient(0deg, rgb(32,32,32,0))";
		...
		setTimeout(()=> { legend.style.maxHeight = `${frame.clientHeight}px`; }, 200);
	}
	...
	CreateLegendElement(color, text) {
		...
		colorBox.style.margin = "4px 1px 2px 6px";
	...
	async InitializeLiveStats() {
		...
		this.liveStatsWebSockets.onclose = ()=> {
			...
			if ("network adapter speed" in this.link) {
		...
		const GenerateGraph = (data, label, type, icon, opt={})=> {
			...
			if (type === "ping") {
				...
				for (let i=0; i<data.length; i++) {
					...
					dot.setAttribute("cx", x);
					dot.setAttribute("cy", y);
					dot.setAttribute("r", data[i].v === 0 ? 1 : 2);
				...
				for (let i=1; i<data.length; i++) {
					...
					dot.setAttribute("cx", x);
					dot.setAttribute("cy", y);
					dot.setAttribute("r", data[i].delta === 0 ? 1 : 2);
	...
	GetSpeedColor(speed) {
		...
		case "2.5 Gbps": return "hsl(65,100%,50%)";
