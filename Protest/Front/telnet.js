class Telnet extends Window {
	...
	HandleMessage(data) {
		for (let i=0; i<data.length; i++) {
			...
			if (!char) {
				...
				char.style.left = `${this.cursor.x * Telnet.CURSOR_WIDTH}px`;
				char.style.top = `${this.cursor.y * Telnet.CURSOR_HEIGHT}px`;
	...
	GetScreenWidth() {
		return parseInt(this.content.clientWidth / Telnet.CURSOR_WIDTH);
	}

	GetScreenHeight() {
		return parseInt(this.content.clientHeight / Telnet.CURSOR_HEIGHT);
	}
