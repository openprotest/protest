class Snmp extends Window {
	...
	constructor(args) {
		...
		this.credentialsProfileInput.value = this.args.profile;
	...
	async GetOid(oid) {
		...
		if (oid.startsWith("1.0.8802.")) {
			const iso8802 = parseInt(oid.substring(9).split(".")[0]);
			...
			const mib2 = parseInt(oid.substring(12).split(".")[0]);
	...
	CreateListItem(oid, type, value, nearestAncestor) {
		...
		typeBox.setAttribute("value", value);
		...
		if (type === "octet-string") {
			...
			if (isBinary) {
				typeBox.className = "snmp-hex";
				valueBox.textContent = this.ValueToHex(value);
			}
			else {
