...
internal static partial class DeviceConfiguration {
    public static byte[] View(Dictionary<string, string> parameters) {
        ...
        try {
            byte[] bytes = File.ReadAllBytes(Path.Join(Data.DIR_CONFIG, file));
    ...
    public static byte[] Save(HttpListenerContext ctx, Dictionary<string, string> parameters, string origin) {
        ...
        try {
            ...
            File.WriteAllBytes(Path.Join(Data.DIR_CONFIG, file), cipher);
        }
        catch (Exception ex){
            Logger.Error(ex);
        ...
        Logger.Action(origin, "Database", $"Modify the device configuration for file: {file}");
    ...
    public static byte[] Fetch(HttpListenerContext ctx, Dictionary<string, string> parameters, string origin) {
        ...
        try {
            ...
            if (firstLine.Trim() != "^" && !firstLine.Contains("bad command name", StringComparison.Ordinal)) { //hpe
                ...
                Logger.Action(origin, "Database", $"Fetch device configuration for file: {host}");
    ...
    public static byte[] ExtractInterfaces(Dictionary<string, string> parameters) {
        ...
        string absolutePath = Path.Join(Data.DIR_CONFIG, file);
