...
internal static class DebitNotes {
    ...
    public static byte[] View(Dictionary<string, string> parameters) {
        ...
        string filename = status switch  {
            "short" => $"{Data.DIR_DEBIT_SHORT}{Data.DIRECTORY_SEPARATOR}{file}",
            "long" => $"{Data.DIR_DEBIT_LONG}{Data.DIRECTORY_SEPARATOR}{file}",
            "returned" => $"{Data.DIR_DEBIT_RETURNED}{Data.DIRECTORY_SEPARATOR}{file}",
            _ => null
        };
    ...
    public static byte[] Create(HttpListenerContext ctx, string initiator) {
        ...
        try {
            ...
            Logger.Action(initiator, $"Create a debit note: {name}");
        }
        catch (Exception ex) {
            Logger.Error(ex);
            return Data.CODE_FAILED.Array;
    ...
    public static byte[] Delete(Dictionary<string, string> parameters, string initiator) {
        ...
        try {
            ...
            Logger.Action(initiator, $"Delete debit note: {filename}");
    ...
    public static byte[] Return(Dictionary<string, string> parameters, string initiator) {
        string file = null;
        string status = null;
        parameters?.TryGetValue("status", out status);
        parameters?.TryGetValue("file", out file);
        ...
        try {
            ...
            File.WriteAllBytes($"{Data.DIR_DEBIT_RETURNED}{Data.DIRECTORY_SEPARATOR}{file}", json);

            File.Delete(filename);

            Logger.Action(initiator, $"Mark a debit note as returned: {file}");
    ...
    public static byte[] ListBanners() {
        DirectoryInfo dir = new DirectoryInfo($"{Configuration.front_path}{Data.DIRECTORY_SEPARATOR}custom");
