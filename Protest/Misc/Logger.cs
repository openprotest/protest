...
internal static class Logger {
    ...
    public static void Error(string ex) {
        lock (syncError)
            try {
                using StreamWriter writer = new StreamWriter($"{Data.DIR_LOG}{Data.DIRECTORY_SEPARATOR}error.log", true, System.Text.Encoding.UTF8);
    ...
    public static void Action(string initiator, string action) {
        new Thread(() => {
            ...
            lock (syncAction)
                try {
                    using StreamWriter writer = new StreamWriter($"{Data.DIR_LOG}{Data.DIRECTORY_SEPARATOR}{dateTime.ToString(Data.DATE_FORMAT_FILE)}.log", true, System.Text.Encoding.UTF8);
    ...
    public static byte[] List() {
        ...
        lock (syncAction) {
            try {
                bytes = File.ReadAllBytes($"{Data.DIR_LOG}{Data.DIRECTORY_SEPARATOR}{DateTime.UtcNow.ToString(Data.DATE_FORMAT_FILE)}.log");
            }
            catch {}
        }
