...
static class Wmi {
    ...
    public static byte[] WmiQuery(string[] para) {
        ...
        string query = "";
        for (int i = 1; i < para.Length; i++) {
            if (para[i].StartsWith("target=")) host = Strings.UrlDecode(para[i].Substring(7));
            if (para[i].StartsWith("q=")) query = Strings.UrlDecode(para[i].Substring(2));
        }
    ...
    public static byte[] WmiKillProcess(string[] para) {
        ...
        for (int i = 1; i < para.Length; i++) {
            if (para[i].StartsWith("target=")) host = Strings.UrlDecode(para[i].Substring(7));
            if (para[i].StartsWith("pid=")) pid = Int32.Parse(Strings.UrlDecode(para[i].Substring(4)));
        }
