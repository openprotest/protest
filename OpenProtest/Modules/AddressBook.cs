...
class AddressBook : Http {
    ...
    private DateTime lastAddressBookBirthday = new DateTime(0);
    private byte[] lastAddressBook;

    private byte[] GetAddressBook(bool force_update = false) {

        if (force_update && DateTime.Now.Ticks - lastAddressBookBirthday.Ticks < 600000000) //less than a minute
        ...
        foreach (DictionaryEntry o in NoSQL.users) {
            ...
            string title = Encoding.UTF8.GetString(NoSQL.GetValue(entry, "TITLE")).Trim();
            string fname = Encoding.UTF8.GetString(NoSQL.GetValue(entry, "FIRST NAME")).Trim();
            string lname = Encoding.UTF8.GetString(NoSQL.GetValue(entry, "LAST NAME")).Trim();
            string depar = Encoding.UTF8.GetString(NoSQL.GetValue(entry, "DEPARTMENT")).Trim();
            string email = Encoding.UTF8.GetString(NoSQL.GetValue(entry, "E-MAIL")).Trim();
            string telep = Encoding.UTF8.GetString(NoSQL.GetValue(entry, "TELEPHONE NUMBER")).Trim();
            string mobno = Encoding.UTF8.GetString(NoSQL.GetValue(entry, "MOBILE NUMBER")).Trim();
            string mobex = Encoding.UTF8.GetString(NoSQL.GetValue(entry, "MOBILE EXTENTION")).Trim();
        ...
        lastAddressBookBirthday = DateTime.Now;
